...
def comparativo_ecommerce(conn_faturamento):
    botao_voltar_menu("comparativo_ecommerce")

    st.markdown("<h2 style='text-align:center'>📊 Comparativos - E-commerce</h2>", unsafe_allow_html=True)
    st.divider()
    ...
    cores = {
        str(ano_passado): "#9699DB",
        str(ano_atual): "#5B5FD6"
    }
    ...
    try:
        ...
        if not df_fat.empty:
            plot_comparativo(df_fat, "Faturamento Mensal E-commerce", "Faturamento")
        else:
    ...
    try:
        ...
        if not df_vendas.empty:
            plot_comparativo(df_vendas, "Quantidade de Vendas Mensal E-commerce", "Quantidade de Vendas")
        else:
