...
def comparativo_santo_andre(conn_faturamento, conn_fluxo):
    botao_voltar_menu("menu")  # ou "comparativo_santo_andre" conforme seu fluxo

    st.markdown("<h2 style='text-align:center'>Comparativos - Santo André</h2>", unsafe_allow_html=True)
    st.divider()
    ...
    cores = {
        str(ano_passado): "#F27687",
        str(ano_atual): "#EB354D"
    }
    ...
    try:
        ...
        if not df_fat.empty:
            plot_comparativo(df_fat, "Faturamento Mensal Santo André", "Faturamento")
        else:
    ...
    try:
        ...
        if not df_fluxo.empty:
            plot_comparativo(df_fluxo, "Fluxo de Pessoas Mensal Santo André", "Fluxo")
        else:
    ...
    try:
        ...
        if not df_vendas.empty:
            plot_comparativo(df_vendas, "Quantidade de Vendas Mensal Santo André", "Quantidade de Vendas")
        else:
    ...
    try:
        if not df_vendas.empty and not df_fluxo.empty:
            ...
            plot_porcentagem(df_conv, "% Conversão de Vendas - Santo André", "Conversão (%)")
        else:
