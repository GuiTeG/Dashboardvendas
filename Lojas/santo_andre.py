...
def mini_card(icone, titulo, valor, cor):
    return f"""
...
def gauge_rentabilidade(valor_atual, meta_percentual):
    fig = go.Figure(go.Indicator(
        ...
        gauge={
            ...
            'steps': [
                {'range': [0, meta_percentual], 'color': "#EB534D"},
...
def mostrar_santo_andre(conn_faturamento, conn_fluxo):
    if st.button("🔙 Voltar ao Menu", key="voltar_menu_santo_andre"):
        st.session_state.pagina = "menu"
        st.rerun()

    col_a, col_b, col_c = st.columns([1, 1, 4])
    with col_a:
        if st.button("👥 Equipe Santo André", key="btn_vendedores_sa"):
            st.session_state["pagina"] = "vendedores_santo_andre"
            st.rerun()
    with col_b:
        if st.button("📲 Vendas Assistidas", key="btn_assistida_sa"):
            st.session_state["pagina"] = "vendas_assistidas_santo_andre"
            st.rerun()

    st.divider()

    # Título da página
    st.markdown("<h2 style='text-align:center'>📍 Santo André - Painel Integrado</h2>", unsafe_allow_html=True)


    col1, col2, _ = st.columns([1, 1, 4])
    with col1:
        data_inicio = st.date_input("Data Inicial", value=date.today(), key="data_inicio_sa")
    with col2:
        data_fim = st.date_input("Data Final", value=date.today(), key="data_fim_sa")
    ...
    query_faturamento = text("""
        ...
        WHERE BIIFDATA BETWEEN :data_inicio AND :data_fim
          ...
          AND BI_TABE.BI_TABEDESC IN ('AUTO SERVICO','VENDEDORES LOJA FÍSICA')
    """)
    result_fat = conn_faturamento.execute(query_faturamento, {"data_inicio": data_inicio, "data_fim": data_fim, "loja_codigo": loja_codigo}).fetchone()
    ...
    with col1:
        ...
        st.markdown(mini_card("🧍‍♂️", "Fluxo de Pessoas", f"{fluxo_total}", cor), unsafe_allow_html=True)
    ...
    with col3:
        ...
        meta_rentabilidade_percentual = 10.93
    ...
    with col2:
        ...
        fig = go.Figure()
        fig.add_trace(go.Bar(x=df_dias["data"], y=df_dias["acima"], name="Acima da meta", marker_color="#EB354D"))
        fig.add_trace(go.Bar(x=df_dias["data"], y=df_dias["abaixo"], name="Abaixo da meta", marker_color="#F27687"))
