...
def mostrar_vendedores_televendas(conn_faturamento):
    st.title("Vendedores - Televendas")

    col1, col2, col3 = st.columns([1, 1, 2])
    with col1:
        data_inicio = st.date_input("Data Inicial", value=date.today(), key="data_inicio_vendedores_televendas")
    with col2:
        data_fim = st.date_input("Data Final", value=date.today(), key="data_fim_vendedores_televendas")
    ...
    query_vendedores = text("""
        ...
        WHERE
            ...
            AND V.BI_CADANOME NOT IN ('WEB', 'VENDEDOR TELEVENDAS', 'MERCADO LIVRE')
    """)
    ...
    if st.button("🔙 Voltar", key="voltar_vendedores_televendas"):
