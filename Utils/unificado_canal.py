...
def mini_card(icone, titulo, valor, cor="#4CAF50"):
    return f"""
...
def mini_gauge_card(label, valor_atual, meta, cor="#4CAF50"):
    ...
    cor_barra = cor if percentual < 100 else "#0DA800"
    return f"""
    <div style="background: white; border-radius: 8px; border: 2px solid {cor}; width: 220px; height: 77px;
...
def mostrar_resumido(nome, cor, loja_codigo, loja_codigo_fluxo, meta_total, conn_faturamento, conn_fluxo):
    ...
    fat = conn_faturamento.execute(text("""
        ...
        WHERE BIIFDATA = CURRENT_DATE AND BIIFGERE = :loja_codigo
          AND BI_TABE.BI_TABEDESC IN ('TELEVENDAS','E-COMMERCE')
    ...
    vendas = conn_faturamento.execute(text("""
        ...
        LEFT JOIN BI_TABE ON BI_TABE.BI_TABECODI = VEND.BI_CADATPVN AND BI_TABE.BI_TABEINDI = 30
        WHERE BIIFDATA = CURRENT_DATE AND BIIFGERE = :loja_codigo
          AND BIIFPVEN = 'S' AND BI_TABE.BI_TABEDESC IN ('TELEVENDAS','E-COMMERCE')
    """), {"loja_codigo": loja_codigo}).fetchone()
...
# --------- INÍCIO DO APP ---------
st.set_page_config(page_title="Painel Resumido - Televendas e E-commerce", layout="wide")
st.title("Painel Resumido - Televendas e E-commerce")
...
with col1:
    mostrar_resumido("Televendas", "#4CAF50", 1133, "5", 1440977, conn_faturamento, conn_fluxo)
with col2:
    mostrar_resumido("E-commerce", "#6C63FF", 1134, "6", 1841774, conn_faturamento, conn_fluxo)
